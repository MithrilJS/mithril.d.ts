...
o.spec("stream", function() {
	o.spec("stream", function() {
		...
		o("can update to undefined", function() {
			var stream: Mithril.Stream<number | undefined> = Stream(1)
			stream(undefined)
	...
	o.spec("combine", function() {
		...
		o("combine lists only changed upstreams in last arg", function() {
			var streams: Mithril.Stream<any>[] = []
			var a = Stream()
			var b = Stream()
		...
		o("combine lists only changed upstreams in last arg with default value", function() {
			var streams: Mithril.Stream<number>[] = []
			var a = Stream(3)
			var b = Stream(5)
			var c = Stream.combine(function(a, b, changed) {
				streams = changed
		...
		o("combine will throw with a helpful error if given non-stream values", function () {
			...
			var thrown = null;
			try {
				// Note we must coerce type to any to force this to compile
				// (and then throw at runtime)
				var b = Stream.combine(spy, [a, ''] as any)
			} catch (e) {
				thrown = e
			}
	...
	o.spec("merge", function() {
		...
		o("calls run callback after all parents are active", function() {
			var value = 0
			var id = function(value: number) {return value}
			var a = Stream<number>()
			var b = Stream<number>()
	...
	o.spec("valueOf", function() {
		...
		o("allows implicit value access in mathematical operations", function() {
			// Note: Typescript cannot yet deal with valueOf or toString overrides
			// so these must be cast to any. :|
			o(Stream(1) as any + Stream(1) as any).equals(2)
		})
	})
	o.spec("toString", function() {
		...
		o("allows implicit value access in string operations", function() {
			o(Stream("a") as any + Stream("b") as any).equals("ab")
		})
	})
	o.spec("toJSON", function() {
		o("works", function() {
			/* Not implemented in public types
	...
	o.spec("map", function() {
		o("works", function() {
			var stream = Stream<number>()
			var doubled = stream["fantasy-land/map"](function(value: number) {return value * 2})
		...
		o("works with undefined value", function() {
			...
			var mapped = stream["fantasy-land/map"](function(value: undefined) {return String(value)})
		...
		o("works with pending stream", function() {
			var stream = Stream(undefined)
			var mapped = stream["fantasy-land/map"](function(value: undefined) {return Stream()})
	...
	o.spec("ap", function() {
		o("works", function() {
			var apply = Stream(function(value: number) {return value * 2})
			var stream = Stream(3)
		...
		o("works with undefined value", function() {
			var apply = Stream(function(value: undefined) {return String(value)})
			var stream = Stream(undefined)
			var applied = stream["fantasy-land/ap"](apply)
	...
	o.spec("fantasy-land", function() {
		o.spec("functor", function() {
			o("identity", function() {
				var stream = Stream(3)
				var mapped = stream["fantasy-land/map"](function(value: number) {return value})

				o(stream()).equals(mapped())
			})
			o("composition", function() {
				function f(x: number) {return x * 2}
				function g(x: number) {return x * x}

				var stream = Stream(3)

				var mapped = stream["fantasy-land/map"](function(value: any) {return f(g(value))})
		...
		o.spec("apply", function() {
			o("composition", function() {
				var a = Stream(function(value: number) {return value * 2})
				var u = Stream(function(value: number) {return value * 3})
				var v = Stream(5)

				var mapped = v["fantasy-land/ap"](u["fantasy-land/ap"](a["fantasy-land/map"](function(f: any) {
		...
		o.spec("applicative", function() {
			o("identity", function() {
				var a = Stream()["fantasy-land/of"](function(value: number) {return value})
				var v = Stream(5)
			...
			o("homomorphism", function() {
				var a = Stream(0)
				var f = function(value: number) {return value * 2}
				var x = 3
			...
			o("interchange", function() {
				var u = Stream(function(value: number) {return value * 2})
				var a = Stream()
				var y = 3

				o(a["fantasy-land/of"](y)["fantasy-land/ap"](u)()).equals(6)
				o(a["fantasy-land/of"](y)["fantasy-land/ap"](u)()).equals(u["fantasy-land/ap"](a["fantasy-land/of"](function(f: any) {return f(y)}))())
			})
		})
	})
})
